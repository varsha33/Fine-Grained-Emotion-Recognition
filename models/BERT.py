...
class Hierarchial_BERT(nn.Module):
    ...
    def __init__(self,resume_path,bert_base_model,batch_size,output_size,hidden_size,grad_check,freeze=False,lstm_hidden_size=256):
        ...
        self.dropout = 0.8
        self.lstm = nn.LSTM(hidden_size,int(hidden_size/2),num_layers=1, dropout=self.dropout, bidirectional=True)
        self.fc1 = nn.Linear(hidden_size,256) ## 256 heuristically chosen
    ...
    def forward(self,text): #here text is utterance_data_list
        ...
        h_0 = Variable(torch.zeros(2, self.batch_size, int(self.hidden_size/2)).cuda())
        c_0 = Variable(torch.zeros(2, self.batch_size, int(self.hidden_size/2)).cuda())
